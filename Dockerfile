...
RUN pacman -Syu --needed --noconfirm clang meson glslang git mingw-w64 wine libunwind base bash base-devel sed git tar curl wget bash gzip sudo file gawk grep bzip2 which pacman systemd findutils diffutils coreutils procps-ng util-linux xcb-util xcb-util-keysyms xcb-util-wm lib32-xcb-util-keysyms glfw-x11
...
# clone, build, then install the lib32-xcb-util AUR package (there is no lib32 xcb-util in the standard Arch repositories)
RUN sudo -u builduser bash -c 'export OLD_PWD=$PWD && cd ~ && git clone https://aur.archlinux.org/lib32-xcb-util.git lib32-xcb-util && cd lib32-xcb-util && makepkg -s --noconfirm && sudo pacman --noconfirm -U lib32-xcb-util* && cd $OLD_PWD'
